...
// 转换器，用于Frame/Mat/IplImage相互转换
        ...
        CanvasFrame cFrame = new CanvasFrame("Joker", CanvasFrame.getDefaultGamma() / grabber.getGamma());
...
//                opencv_imgproc.putText(mat, "joker!", point2, opencv_imgproc.CV_FONT_VECTOR0, 2.2, scalar2, 1, 0, false);
                // 翻转字体，文字平滑处理（即反锯齿）
//                opencv_imgproc.putText(mat, "joker!", point3, opencv_imgproc.CV_FONT_VECTOR0, 2.2, scalar2, 1, 20, true);
...
//                opencv_imgproc.putText(mat, sdf.format(new Date()), point1, opencv_imgproc.CV_FONT_ITALIC, 0.8, scalar1, 2, 20, false);
                ...
                if (index == 0) {
                    // 保存第一帧图片到本地
                    opencv_imgcodecs.imwrite("E:\\pic_save_all\\1.jpg", mat);
                }
                // 释放Mat资源
                mat.release();
                mat.close();
